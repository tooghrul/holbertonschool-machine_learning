#!/usr/bin/env python3
"""Module for performing convolution on images with multiple channels."""
...
def convolve_channels(images, kernel, padding='same', stride=(1, 1)):
    """
    Perform a convolution on images with channels using a given kernel.

    Args:
        images (numpy.ndarray): Array of shape (m, h, w, c) containing multiple
            images, where m is the number of images, h is the height in pixels,
            w is the width in pixels, and c is the number of channels.
        kernel (numpy.ndarray): Array of shape (kh, kw, c) containing the
            kernel for the convolution, where kh is the height, kw is the width
            of the kernel, and c is the number of channels (must match images).
    ...
    for i in range(output_h):
        for j in range(output_w):
            # Extract the receptive field (patch) for all images and channels
            # Shape: (m, kh, kw, c)
            patch_start_h = i * sh
            patch_start_w = j * sw
            patches = padded[
            ...
            # Kernel shape (kh, kw, c) broadcasts with patches (m, kh, kw, c)
            # Then sum over the kernel spatial dimensions and channels
            # (axes 1, 2, and 3)
            # Result shape: (m,) - one convolution value per image
            output[:, i, j] = np.sum(patches * kernel, axis=(1, 2, 3))
